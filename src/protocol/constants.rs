...
/// 选取原则: 行情数据面验证可用优先, 其余为快速元数据兜底
///
/// 2026-09-24 重排 (数据面四级筛查实测):
///   - 2026-07 服务端协议更新后行情报文族大面积返空, 但国泰君安集群
///     (117.34.114.x) 幸存: 实时行情/日K 返回真实数据 (600519 日K到 09-23)
///   - 数据可用 7 台全部纳入 PRIMARY; 另选 3 台 TCP/API 最快的返空机兜底
///     (元数据类接口 list/finance/xdxr 在返空机上仍然正常)
///   - 注意: 国泰君安 7 台同属 117.34.114.0/24, 有集群同挂风险;
///     若数据源恢复应重新筛查 (python -m tdxrs servers) 并重排
///
/// 历史修正 (2026-07-08 / 2026-07-17): 移除华林4/海通系故障机/广发1 等返空机
pub const PRIMARY_SERVERS: &[(&str, &str, u16)] = &[
    // ── 行情数据面验证可用 (2026-09-24 筛查, 国泰君安集群) ──
    ...
    // ── 快速元数据兜底 (行情返空但 TCP/握手/API 最快, 数据源恢复前仅元数据可用) ──
